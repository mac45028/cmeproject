...
public:
	...
	friend std::ostream& operator<<(std::ostream& o, const varint_u<size> &v) {
		if(o.flags() & std::ios_base::hex) {
			const std::ios_base::fmtflags orig = o.flags();

			const int num_word_bits = 64;
			const int num_top_bits = size % num_word_bits;
			const int num_words = size / num_word_bits;

				if(o.flags() & std::ios_base::showbase)
					o << (o.flags() & std::ios_base::uppercase ? "0X" : "0x");

			if (num_top_bits != 0)
				o << std::setw(0) << v.get_bits(num_words * num_word_bits, num_top_bits);
		
			for (int i = num_words - 1; i >= 0; i--)
				o << std::setw(num_word_bits/8*2) << std::setfill('0') << v.get_bits(i * num_word_bits, num_word_bits);
	...
	uint64_t get_bits(int base, int length) const {
		...
		uint m_word_idx = l_word_idx + 1;
		uint u_word_idx = (base + length) / s_chunk_bits;
		...
		uint value_offset = l_length;

		// Middle chunk
		// Depending on s_chunk_bits there could be more than one middle chunk.
		// We assume there's only one chunk anyway, because a for loop could potentially
		// degrade performance and a LOT of other code would break if s_chunk_bits
		// ever changed.
		bool has_middle = m_word_idx < s_num_chunks && u_word_idx > m_word_idx;

		if (has_middle) {
			result |= m_store[m_word_idx] << value_offset;

			uint m_length = s_chunk_bits;
			value_offset += m_length;
		}

		// Upper chunk
		bool has_upper = u_word_idx < s_num_chunks && u_word_idx > l_word_idx;

		if (has_upper) {
			result |= m_store[u_word_idx] << value_offset;
		}
	...
	void set_bits(int base, int length, uint64_t value) {
		...
		// Apply the new bits, lower chunk:
		uint l_offset = base%s_chunk_bits;
		uint l_length = s_chunk_bits-l_offset;
		...
		if (has_middle) {
			uint m_length = s_chunk_bits;
			uint64_t m_mask = s_mask_low;

			m_store[m_word_idx] &= ~m_mask;
			m_store[m_word_idx] |= (value >> value_offset) & m_mask;
		...
		if (has_upper) {
			uint u_length = (base + length) % s_chunk_bits;
			uint64_t u_mask = BIT(u_length)-1;

			m_store[u_word_idx] &= ~u_mask;
			m_store[u_word_idx] |= (value >> value_offset) & u_mask;
		}
	}
